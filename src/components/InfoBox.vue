<template>
  <div class="info-box flex my-3">
    <!-- Barra lateral colorida usando as cores exatas -->
    <div class="border-l-4" :style="{ borderColor: typeColors[type] }"></div>
    ...
    <div
      ...
      :style="{
        backgroundColor: isDark
          ? `${typeColors[type]}10` // Adiciona 10% de opacidade no tema escuro
          : `${typeColors[type]}05`, // Adiciona 5% de opacidade no tema claro
      }"
    >
      <!-- Título com ícone -->
...
export default {
  ...
  data() {
    return {
      // Cores exatas para cada tipo
      typeColors: {
        note: "#4493F8", // Azul
        tip: "#3FB950", // Verde
        important: "#AB7DF8", // Roxo
        warning: "#D29922", // Amarelo/âmbar exato da imagem
        caution: "#F85149", // Vermelho
      },
      // Mapeamento de tipos para títulos em português
      titleMap: {
        note: "Nota",
        tip: "Dica",
  ...
  methods: {
    // Retorna o SVG para cada tipo de alerta
    getIcon(type) {
      const iconColor = this.typeColors[type]; // Usa a cor exata do tipo
