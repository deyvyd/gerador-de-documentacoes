...
// Componente de modal para diferentes tipos de mensagens (note, tip, important, warning, caution)
window.AppComponents.ModalMessage = {
  props: {
    show: Boolean,
    titulo: {
      type: String,
    ...
    mensagem: {
      type: String,
      default: "Conteúdo da mensagem.",
    },
    tipo: {
      type: String,
      default: "note", // note, tip, important, warning, caution
      validator: (value) =>
    ...
    larguraPersonalizada: {
      type: String,
      default: "max-w-md", // Classes do Tailwind para largura (max-w-sm, max-w-md, max-w-lg, max-w-xl, etc.)
    },
    // Nova propriedade para controlar se o clique fora do modal deve fechá-lo
    fecharAoClicarFora: {
      type: Boolean,
      default: false, // Por padrão, não fecha ao clicar fora
    },
  },
  ...
  computed: {
    ...
    // Classes e informações baseadas no tipo
    modalConfig() {
      const configs = {
        note: {
    ...
    // Classes para o botão principal
  ...
  methods: {
    ...
    // Fechar o modal quando clicar fora dele (apenas se fecharAoClicarFora for true)
  ...
  mounted() {
    ...
    // Prevenir a rolagem do body quando o modal estiver aberto
    if (this.show) {
      document.body.classList.add("modal-open");
    }
  },
  updated() {
    // Atualizar classe no body quando o estado do modal mudar
    if (this.show) {
      document.body.classList.add("modal-open");
    } else {
  ...
  beforeUnmount() {
    ...
    // Garantir que a classe seja removida ao desmontar o componente
    document.body.classList.remove("modal-open");
  },
  template: `
    <div v-if="show" 
      ...
      <div :class="['modal-message', 'p-6', 'rounded-lg', 'shadow-xl', 'w-full', 'mx-4', 'transform', 'transition-all', 'duration-300', 'scale-100', larguraPersonalizada, isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-800']">
        <!-- Cabeçalho do modal -->
        <div class="flex items-start mb-4 gap-4">
          <!-- Ícone baseado no tipo -->
          <div :class="['p-3', 'rounded-full', modalConfig.colorClass]" v-html="modalConfig.icon"></div>
