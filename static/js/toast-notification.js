...
// Componente de notificação toast aprimorado para usar os estilos do toast.css
window.AppComponents.ToastNotification = {
  props: {
    mensagens: {
      type: Array,
      default: () => [],
    ...
    <div :class="containerClasses">
      <transition-group name="toast-list">
        <div
          ...
          :key="msg.id"
          :class="getToastClasses(msg)"
          @mouseenter="pausarTemporizador(msg.id)"
          @mouseleave="retomarTemporizador(msg)"
          role="alert"
          tabindex="0"
        >
          <!-- Ícone -->
          <div 
            v-if="mostrarIcone" 
            v-html="getToastConfig(msg.tipo).icon"
            class="toast-icon-container"
          ></div>
          
          <!-- Conteúdo -->
          <div class="toast-content">
            <h4 v-if="msg.titulo" class="toast-title">{{ msg.titulo }}</h4>
            <p class="toast-message">{{ msg.texto }}</p>
          </div>
          
          <!-- Botão fechar -->
          <button 
            v-if="mostrarFechar"
            @click.prevent="fecharToast(msg.id)"
            class="toast-close"
            aria-label="Fechar"
          >
          ...
          <!-- Barra de progresso -->
          <div v-if="mostrarProgresso && progressos[msg.id] !== undefined" class="toast-progress">
            <div 
              class="toast-progress-bar" 
              :style="{ width: progressos[msg.id] + '%' }"
            ></div>
          </div>
        </div>
      </transition-group>
  ...
  `,
  beforeDestroy() {
    // Limpar todos os temporizadores ao destruir o componente
    Object.keys(this.timers).forEach((id) => {
      clearInterval(this.timers[id]);
    });
  },
