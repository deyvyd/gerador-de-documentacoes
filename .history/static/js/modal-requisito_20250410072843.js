...
window.AppComponents.ModalRequisito = {
  ...
  methods: {
    ...
    salvarRequisito() {
      ...
      // 1. Nome
      if (!this.requisito.nome) {
        this.tabAtiva = 0; // Aba de informações básicas
        setTimeout(() => {
          const elemento = document.getElementById("req-nome");
          if (elemento) {
      ...
      if (!this.requisito.descricao) {
        ...
        setTimeout(() => {
          ...
          if (editorContainer) {
            ...
            // Modificar esta parte para evitar o erro de foco no editor
            try {
              // Verificar se o editor existe e se tem o elemento root antes de tentar focar
              const editorElement = document.querySelector(".ql-editor");
              if (editorElement) {
                editorElement.focus();
              }
          ...
          <div v-if="tabAtiva === 0" class="space-y-3" ref="abaConteudo0">
            <div class="grid grid-cols-4 gap-4">
              <div class="form-group col-span-3 mb-0">
                <label for="req-nome" class="form-label mb-1" :class="isDarkMode ? 'text-gray-200' : 'text-gray-700'">
                  Nome
