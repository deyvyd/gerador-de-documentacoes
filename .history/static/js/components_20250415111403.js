...
const InfoButton = {
  ...
  template: `
    <button class="info-toggle">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        class="h-6 w-6 text-gray-800 dark:text-white hover:text-blue-500 dark:hover:text-blue-400 transition-colors duration-200"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
      >
      ...
      <div class="info-tooltip">
        ...
        <div class="tooltip-content text-left">
          <p class="font-medium mb-2 text-left">{{ tooltipTitle }}</p>
          <ul class="space-y-1 text-sm text-left">
            <li v-for="(item, index) in tooltipItems" :key="index" class="text-left">• {{ item }}</li>
          </ul>
        </div>
      </div>
    </button>
