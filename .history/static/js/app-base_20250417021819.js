...
window.AppBase = (function() {
  ...
  const getSharedComputed = (context) => {
    return {
      filteredAutores() {
        return context.filterAutores();
      }
    };
  };
  ...
  const methods = {
    ...
    selectAutor(autor) {
      if (!this.selectedAutores.some((selected) => selected.nome === autor.nome)) {
    ...
    updateFormDataAutores() {
      ...
      this.formData.iniciaisAutor = `${iniciais.join(", ")} e ${ultimaIniciais}`;
    },
    ...
    aplicarErroTemporarioAutores(duracao = 3000) {
      ...
      let inputContainer = basicFormSection.$el.querySelector(".autor-container");
    ...
    validarCamposObrigatorios() {
      ...
      for (const campo of camposObrigatorios) {
        ...
        if (!valor || valor.trim() === "") {
          ...
          if (elemento) {
            ...
            this.notificationService.show(`O campo ${campo.label} é obrigatório`, "error");
            this.isLoading = false;
            return false;
          }
        }
      ...
      if (dataInicio > dataFim) {
        ...
        if (elemento) {
          this.aplicarErroTemporario(elemento);
          this.notificationService.show("A data de início não pode ser posterior à data de fim", "error");
          this.isLoading = false;
          return false;
        }
      }
    ...
    prepareFormData(onlyJson = false) {
      const formData = new FormData();

      // Adiciona formatos ao FormData
      formData.append("gerar_json", "true");
      formData.append("gerar_docx", onlyJson ? "false" : (this.formData.gerarDocx ? "true" : "false"));
      formData.append("gerar_pdf", onlyJson ? "false" : (this.formData.gerarPdf ? "true" : "false"));
