...
export default {
    ...
    toggleTheme() {
      ...
      resetAutorFieldError() {
        const basicFormSection = this.$refs.basicFormSection;
        if (!basicFormSection || !basicFormSection.$el) return;
      
        try {
          // Tentar diferentes seletores para encontrar o container de autores
          const selectors = [".form-group:nth-child(4) .form-input", ".autor-container"];
          ...
          for (const label of labels) {
            if (label.textContent.includes("Autor(es)")) {
              const container =
                ...
                label.nextElementSibling?.querySelector(".autor-container");
              if (container) {
                container.classList.remove("campo-erro");
                return;
              }
          ...
          console.warn("Não foi possível acessar o container de autores:", error);
        }
      },
