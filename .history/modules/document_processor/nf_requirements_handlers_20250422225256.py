...
def substituir_por_texto_padrao(doc):
    ...
    for i, p in enumerate(doc.paragraphs):
        if "3 - Requisitos Não-Funcionais" in p.text:
            ...
            if "RNF-" in p.text or "[DESCRICAO_RNF]" in p.text:
                indices_a_remover.append(i)
                logger.info(f"Marcando parágrafo de RNF para remoção: índice {i}")
    
    if indice_titulo_secao is None:
        logger.error("Não foi possível encontrar o título da seção de RNF")
        return
    
    # Remover parágrafos marcados de trás para frente para não afetar os índices
    body = doc._body._body
    for i in sorted(indices_a_remover, reverse=True):
        paragrafo = doc.paragraphs[i]
        if paragrafo._p.getparent() is not None:
            body.remove(paragrafo._p)
            logger.info(f"Removido parágrafo de RNF no índice {i}")
    ...
    # Remover o parágrafo de sua posição atual
    elemento_padrao = paragrafo_padrao._p
    paragrafo_padrao._p.getparent().remove(paragrafo_padrao._p)
    
    # Inserir após o título da seção
    body.insert(indice_titulo_secao + 1, elemento_padrao)
