...
def processar_requisitos_nao_funcionais(doc, requisitos_nao_funcionais):
    ...
    posicao_insercao = indice_titulo_secao + len(requisitos_nao_funcionais)  # Após o título da seção
    logger.info(f"Inserindo novo parágrafo para requisito não funcional no índice {posicao_insercao}")
    logger.info(f"A quantidade de requisitos é: {len(requisitos_nao_funcionais)}")
